...
void ui_render(const MemoryStats *mem, const DiskStats *disk, double cpu,
    ...
    mvprintw(1, 0, "RAM:  %5.2f / %5.2f GB (%4.1f%%)", mem->used_gb,
             mem->total_gb, mem->percent);
    mvprintw(2, 0, "Disk: %5.2f / %5.2f GB (%4.1f%%)", disk->used_gb,
             disk->total_gb, disk->percent);
    ...
    for (int i = 0; i < max_display && (i + scroll_offset) < proc_count; i++) {
        ...
        mvprintw(start_row + 1 + i, 0, " %-8d | %-9c | %-11ld | %s",
                 procs[idx].pid, procs[idx].state, procs[idx].rss_mb,
                 procs[idx].name);
    ...
    mvprintw(LINES - 1, 0,
             "[Proccesses: %d/%d] | [Arrows]: Navigation | [K]: Kill process | "
             "[Q]: Quit",
             selected_idx + 1, proc_count);
    refresh();
}

int ui_get_input(void) { return getch(); }
